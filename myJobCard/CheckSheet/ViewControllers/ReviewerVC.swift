...
class ReviewerVC: UIViewController,UITableViewDelegate,UITableViewDataSource,UISearchBarDelegate{
    ...
    override func viewDidLoad() {
        ...
        ODSUIHelper.setBorderToView(view: self.reviewerCheckSheetView, borderWidth: 1.0, cornerRadius: 0.0, borderColor: .lightGray)
        reviewerCheckSheetSearchField.compatibleSearchTextField.backgroundColor = UIColor.white
        reviewerCheckSheetSearchField.setImage(UIImage(), for: .search, state: .normal)
    ...
    func getPreviewCheckSheet(index:Int) {
        menuDataModel.uniqueInstance.presentCheckSheetViewerScreen(vc: self, isFromScrn: "Reviewer", isFromEdit: true)
    }
    //...End...//
}
