...
import FormsEngine
import mJCLib
class DashboardStyle3: UIViewController,UICollectionViewDelegate,UICollectionViewDataSource,ChartViewDelegate,CustomNavigationBarDelegate,UITableViewDelegate,UITableViewDataSource, CreateUpdateDelegate{
    ...
    override func viewDidLoad() {
        ...
        if DOWNLOAD_CREATEDBY_WO == "X" {
            WoFilterArray.append("Created_or_Assigned".localized())
        }
        if DOWNLOAD_CREATEDBY_NOTIF == "X" {
            NoFilterArray.append("Created_or_Assigned".localized())
        }
        ...
        ODSUIHelper.setBorderToView(view: self.addButton, borderColor: UIColor(named: "mjcViewUIBorderColor") ?? UIColor.blue)
        ...
        detailsTableView.estimatedRowHeight = 50.0
        ...
        ScreenManager.registerDBStyle3ListCell(tableView: self.detailsTableView)
    }
    ...
    func getCompletedWorkorder(from:String){
        if from == "Today"{
            ...
            let filterdate1 = ODSDateHelper.getDateFromString(dateString:  Date().toString(format: .custom(localDateFormate)) , dateFormat: localDateFormate)
            
            let namePredicate = NSPredicate(format: "SELF.SchdFnshDate == %@",filterdate1 as CVarArg);
            let namePredicate1 = NSPredicate(format: "SELF.TechCompletionDate == %@ and SELF.UserStatus == %@",filterdate1 as CVarArg,"COMP");
            ...
            DispatchQueue.main.async{
                if workorderfortoday.count > 0{
                    ...
                    self.totalMaintenanceWoLabel.text = "Today's completed Work orders : \(percent)%"
                    self.completionText.isHidden = true
                }else{
                    self.horizonatalchart.isHidden = true
                    self.completionText.isHidden = false
                    self.view.bringSubviewToFront(self.completionText)
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if tableView.tag == 1100{
            let cell = ScreenManager.getDBStyle3ListCell(tableView: tableView)
                if let  selectedclass = self.selectedChartArr[indexPath.row] as? WoHeaderModel{
...
//                    }
                    ...
                    return cell
                }else if  let  selectedclass = self.selectedChartArr[indexPath.row] as? NotificationModel{
                    cell.woNumLabel.text = selectedclass.Notification
                    cell.descriptionLabel.text = selectedclass.ShortText
                    
                    if selectedclass.RequiredEndDate != nil{
                        let date = selectedclass.RequiredEndDate!.toString(format: .custom(localDateFormate))
                        cell.dateLabel.text = date
                    }else{
                        cell.dateLabel.text = ""
                    }
                    ...
                    cell.priorityImg.image = myAssetDataManager.getPriorityImage(priority: selectedclass.Priority)
                    cell.priorityLabel.text = ""
                    cell.TypeLabel.text = selectedclass.NotificationType
                    cell.locationLabel.text = selectedclass.PostalCode
                    return cell
                }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if tableView.tag == 1100{
            if selectedChartArr[indexPath.row] is WoHeaderModel{
                totalWorkorders.removeAllObjects()
                filterWorkorders.removeAllObjects()
                currentMasterView = "WorkOrder"
                totalWorkorders.addObjects(from: self.WorkOrderArray)
                filterWorkorders.add(selectedChartArr[indexPath.row])
                singleWorkOrder = selectedChartArr[indexPath.row] as! WoHeaderModel
                selectedworkOrderNumber = singleWorkOrder.WorkOrderNum
                
            }else if selectedChartArr[indexPath.row] is NotificationModel{
                currentMasterView = "Notification"
                ...
                filterNotifications.add(selectedChartArr[indexPath.row])
            }
...
//                        else if WithdrawalQty > 0 {
//                            incompleteWithdrawnCount+=1
//                        }
    ...
    @IBAction func newdashBoardButtonAction(_ sender: Any) {
     ...
     public func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell
    {
        ...
        var cell = DashboardCollectionViewCell()
            ...
            else if dashBoardArray[indexPath.row] == "DASH_NOTI_TILE"
            {
                ...
                cell.centerImage.image = UIImage(named: "asset_hierarchy")
                cell.AddImage.isHidden = true
                cell.bringSubviewToFront(cell.AddImage)
                cell.addButton.isHidden = true
