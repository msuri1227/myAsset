...
class NotificationOverViewCell: UITableViewCell {
    ...
    var onlineNotiOverviewViewModel = OnlineSearchNotificationOverviewViewModel()
    var onlineNotifOverviewModelClass : NotificationModel? {
        didSet{
            onlineNotificationOverViewConfiguration()
        }
    }
    ...
    // Online Notification Overview
    func onlineNotificationOverViewConfiguration() {
        mJCLogger.log("Starting", Type: "info")
        if let onlineNotifOverviewModelClass = onlineNotifOverviewModelClass {
            overViewDescriptionLabel.text = onlineNotifOverviewModelClass.ShortText
            
            if onlineNotifOverviewModelClass.Notification != ""{
                
                overViewNotificationLabel.text = onlineNotifOverviewModelClass.Notification
                
            }
            
            overViewPriorityLabel.text = onlineNotifOverviewModelClass.Priority
            overViewNotificationTypeLabel.text = onlineNotifOverviewModelClass.NotificationType
            
            let breakdownDur = onlineNotifOverviewModelClass.Breakdown
            if onlineNotifOverviewModelClass.Unit == "H" ||  onlineNotifOverviewModelClass.Unit == "h" {
                overViewBreakdownDurationLabel.text = String(breakdownDur ) + " " + "Hours".localized()
            }
            else {
                overViewBreakdownDurationLabel.text = String(breakdownDur ) + " " + "Units".localized()
            }
            
            overViewCatalogTypeLabel.text = onlineNotifOverviewModelClass.CatalogType
            overViewCodeGroupLabel.text = onlineNotifOverviewModelClass.CodeGroup
            overViewCodingLabel.text = onlineNotifOverviewModelClass.Coding
            
            overViewFunctionalLocationLabel.text = onlineNotifOverviewModelClass.FunctionalLoc
            overViewEquipementLabel.text = onlineNotifOverviewModelClass.Equipment
            overViewEquipementButton.tag = indexPath.row
            overViewFunctionalLocationButton.tag = indexPath.row
            overViewFunctionalLocationButton.setTitle(onlineNotifOverviewModelClass.FunctionalLoc, for: .normal)
            
            overViewFunctionalLocationButton.addTarget(self, action: #selector(onlineNotificationFuncLocButtonAction(sender:)), for: .touchUpInside)
            overViewEquipementButton.addTarget(self, action: #selector(onlineNotificationEquipmentButtonAction(sender:)), for: .touchUpInside)
            
            overViewEquipementButton.setTitle(onlineNotifOverviewModelClass.Equipment, for: .normal)
            
            if onlineNotifOverviewModelClass.Breakdown == "X" || onlineNotifOverviewModelClass.Breakdown == "X" {
                breakdownButton.isHidden = false
            }
            else {
                breakdownButton.isHidden = true
            }
    ...
    //Function Location Button Action..
    @objc func onlineNotificationFuncLocButtonAction(sender : UIButton) {
        
        mJCLogger.log("Starting", Type: "info")
        
        mJCAlertHelper.showAlert(onlineNotiOverviewViewModel.vcOnlineNotiOverview!, title: MessageTitle, message: "Data_not_found".localized(), button: okay)
      
        mJCLogger.log("Ended", Type: "info")
    }
    
    //Equipment Button Action..
    @objc func onlineNotificationEquipmentButtonAction(sender : UIButton) {
