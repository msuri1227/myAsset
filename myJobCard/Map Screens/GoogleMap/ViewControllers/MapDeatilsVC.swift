...
class MapDeatilsVC: UIViewController,UIGestureRecognizerDelegate,CLLocationManagerDelegate,GMSMapViewDelegate,UISearchBarDelegate, CustomNavigationBarDelegate, SlideMenuControllerSelectDelegate,filterDelegate{
    ...
    override func viewDidLoad() {
        ...
        ODSUIHelper.setCornerRadiusToImgView(imageView: self.workOrderStatusImageView, cornerRadius: self.workOrderStatusImageView.frame.size.width / 2)
        ...
        if DeviceType == iPhone {
            ODSUIHelper.setBorderToView(view: self.searchInView, borderWidth: 1.0, cornerRadius: 0, borderColor: .lightGray)
            ODSUIHelper.setRoundLabel(label: self.filterCountLbl)
    ...
    private func updateSlideMenu() {
        ...
        if isSupervisor == "X" {
            myAssetDataManager.uniqueInstance.leftViewController.sideMenuListArr = ["Work_Orders".localized(),"Notifications".localized(),"Supervisor_View".localized(),"Team".localized(),"Time_Sheet".localized(),"Master_Data_Refresh".localized(), "Asset_Map".localized(),"Settings".localized(), "Log_Out".localized()]
            ...
            myAssetDataManager.uniqueInstance.leftViewController.sideMenuListArr = ["Work_Orders".localized(),"Notifications".localized(),"Time_Sheet".localized(),"Master_Data_Refresh".localized(), "Asset_Map".localized(),"Settings".localized(), "Log_Out".localized()]
    ...
    @IBAction func moreInfoButtonAction(sender: AnyObject) {
        mJCLogger.log("Starting", Type: "info")
        if DeviceType == iPad {
            menuDataModel.uniqueInstance.presentListSplitScreen(type: "WorkOrder", isFromMapScrn: true)
        }else{
