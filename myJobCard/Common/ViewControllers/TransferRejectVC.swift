...
class TransferRejectVC: UIViewController,UITextViewDelegate,UITextFieldDelegate,personResponsibleDelegate,timeSheetDelegate,viewModelDelegate{
    ...
    func setViewLayout(view:UIView) {
        mJCLogger.log("Starting", Type: "info")
        ODSUIHelper.setBorderToView(view: view, borderWidth: 1.5, cornerRadius: 3.0, borderColor: appColor)
        mJCLogger.log("Ended", Type: "info")
    }
                    ...
                    if status == "MOBI" || status == "ACCP"{
                        ...
                        if let workFlowObj = workFlowResp as? LtWorkFlowModel {
                            if workFlowObj.ActionType == "Screen" {
                                menuDataModel.presentCreateTimeSheetScreen(vc: self, isFromScrn: "StatusChange", delegateVC: self, selectedWONumber: selectedworkOrderNumber, statusCategoryCls: self.statusCategoryCls)
                            }
                        }
                    }
                }else{
