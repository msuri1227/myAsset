...
class WorkOrderMainVC: UIViewController,CustomNavigationBarDelegate,SlideMenuControllerSelectDelegate {
    ...
    var ComponentsVc : ComponentListVC?
    ...
    func SlideMenuSelected(index: Int, title: String,menuType menutype:String){
        ...
        if title == "Work_Orders".localized() {
            ...
            seletectedMenu = "Components".localized()
            ComponentsVc = ScreenManager.getComponentListScreen()
            addChild(ComponentsVc!)
            ComponentsVc?.view.frame = CGRect(x: 0, y: 0, width: self.mainHolderView.frame.size.width, height: self.mainHolderView.frame.size.height)
            self.mainHolderView.addSubview(ComponentsVc!.view)
            ...
            seletectedMenu = "Record_Points".localized()
            let operationsVC = ScreenManager.getRecordPointsScreen()
            addChild(operationsVC)
            operationsVC.view.frame = CGRect(x: 0, y: 0, width: self.mainHolderView.frame.size.width, height: self.mainHolderView.frame.size.height)
            self.mainHolderView.addSubview(operationsVC.view)
            operationsVC.didMove(toParent: self)
        ...
        }else if title == "History".localized() {
            seletectedMenu = "History".localized()
            let operationsVC = ScreenManager.getHistoryAndPendingScreen()
            operationsVC.isfrom = "History"
            addChild(operationsVC)
            operationsVC.view.frame = CGRect(x: 0, y: 0, width: self.mainHolderView.frame.size.width, height: self.mainHolderView.frame.size.height)
            self.mainHolderView.addSubview(operationsVC.view)
            operationsVC.didMove(toParent: self)
        ...
        }else if title == "Pending".localized() {
           seletectedMenu = "Pending".localized()
            let operationsVC = ScreenManager.getHistoryAndPendingScreen()
            operationsVC.isfrom = "Pending"
            addChild(operationsVC)
            operationsVC.view.frame = CGRect(x: 0, y: 0, width: self.mainHolderView.frame.size.width, height: self.mainHolderView.frame.size.height)
            self.mainHolderView.addSubview(operationsVC.view)
            operationsVC.didMove(toParent: self)
        ...
        }else if title == "Items".localized() {
            
            seletectedMenu = "Items".localized()
            let operationsVC = ScreenManager.getNotificationItemListScreen()
            addChild(operationsVC)
            operationsVC.isfrom = "Items"
            operationsVC.view.frame = CGRect(x: 0, y: 0, width: self.mainHolderView.frame.size.width, height: self.mainHolderView.frame.size.height)
            self.mainHolderView.addSubview(operationsVC.view)
            operationsVC.didMove(toParent: self)
            ...
            seletectedMenu = "Activities".localized()
            let operationsVC = ScreenManager.getNotificationItemListScreen()
            addChild(operationsVC)
            ...
            seletectedMenu = "Tasks".localized()
            let operationsVC = ScreenManager.getNotificationItemListScreen()
            addChild(operationsVC)
    ...
    func threedotmenuButtonClicked(_ sender: UIButton?){
        ...
        menudropDown.selectionAction = { [unowned self] (index: Int, item: String) in
            if item == "Create_Notification".localized() {
                ...
                menuDataModel.presentCreateWorkorderScreen(vc: self, isFromEdit: true)
                ...
                if isActiveWorkOrder == true {
                    if self.operationsVC!.totalOprationArray.count > 0 {
                        ...
                        if let workFlowObj = workFlowResp as? LtWorkFlowModel {
                            if workFlowObj.ActionType == "Screen" {
                                menuDataModel.presentCreateOperationScreen(vc: self, operationCls: self.operationsVC?.singleOperationArray[0] ?? WoOperationModel())
                            }else if workFlowObj.ActionType == "FORM" || workFlowObj.ActionType == "Form"{
                                myAssetDataManager.uniqueInstance.getFormsPage(formDetails: workFlowObj.ActionKey, screen: self, statusCategoryCls: StatusCategoryModel(), formFrom: "")
                            }else{
                                myAssetDataManager.uniqueInstance.showtoastmsg(actionTitle: alerttitle, msg: "WorkFlowError".localized())
            ...
            }else if item == "Add_Component".localized() {
