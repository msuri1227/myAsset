...
//            self.dueDateTimeButton.isEnabled = false
            ...
            if CREATE_WORKORDER_WITH_OPERATION == true{
                ...
                if let workFlowObj = workFlowResp as? LtWorkFlowModel {
                    ...
                    if workFlowObj.ActionType == "Screen" {
                        mJCLoader.stopAnimating()
                        DispatchQueue.main.async {
                            menuDataModel.presentCreateOperationScreen(vc: self, isScreen: "WorkOrderCreate", delegate: true, operationNmbr: "L010", newWorkOrderNotes: self.noteTextView.text, propertyWOArray: self.property)
                        }
                    }
                }
            }else {
    ...
    @IBAction func checkOnlineWorkOrdersButtonAction(_ sender: Any) {
        showOnlineSearchPopUp(searchType: "WorkOrder")
    }
