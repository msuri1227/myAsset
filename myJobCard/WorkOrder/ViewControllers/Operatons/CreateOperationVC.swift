...
//                    self.functionalLocationScantButton.isHidden = true
    ...
    @IBAction func equipmentSelectButtonAction(sender: AnyObject) {
        ...
        if self.workCenterTextField.text != "" || self.plantTextField.text != nil {
            workCenterStr = self.workCenterTextField.text
    ...
    @IBAction func functionalLocationSelectButtonAction(sender: AnyObject) {
        var workCenterStr : String? = ""
        var planningPlantStr : String? = ""
        if self.workCenterTextField.text != "" || self.workCenterTextField.text != nil {
            workCenterStr = self.workCenterTextField.text
            planningPlantStr = self.workCenterTextField.text
        }
        menuDataModel.uniqueInstance.presentFlocEquipHierarchyScreen(vc: self, select: "FunctionalLocation", delegateVC: self, workCenter: workCenterStr, selectedFuncLoc: functionalLocationTextField.text, maintenancePlant: planningPlantStr)
    }
    @IBAction func personResponsibleScanButtonAction(_ sender: Any) {
        if self.personResponsibleListArray.count > 0 {
            menuDataModel.uniqueInstance.presentPersonResponsibleListScreen(vc: self, delegateVC: self)
        }
    }
