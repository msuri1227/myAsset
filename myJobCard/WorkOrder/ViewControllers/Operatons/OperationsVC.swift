...
class OperationsVC: UIViewController,UITableViewDataSource,UITableViewDelegate,CustomNavigationBarDelegate,CreateUpdateDelegate,operationCreationDelegate,listSelectionDelegate{
    ...
    func setIPhoneBasicView(){
        ...
        if isfromsup == "Supervisor" {
            ...
            if WORKORDER_ASSIGNMENT_TYPE == "1" || WORKORDER_ASSIGNMENT_TYPE == "3"{
                let view = CustomNavHeader_iPhone.init(viewcontroller: self, leftMenu: true, leftTitle: "\(selectedworkOrderNumber) / \(selectedOperationNumber)" , NewJobButton: true, refresButton: true, threedotmenu: true,leftMenuType:"Back")
                self.iPhoneHeader.addSubview(view)
                if flushStatus == true{
                    view.refreshBtn.showSpin()
                }
    ...
    @IBAction func followonWoCreateAction(_ sender: Any) {
        ...
        if operationVCModel.AllowedFollOnObjTypArray.count > 0{
            DispatchQueue.main.async {
                menuDataModel.presentCreateWorkorderScreen(vc: self, isScreen: "WorkOrder", AllowedFollOnObjTypArray: self.operationVCModel.AllowedFollOnObjTypArray, fromfollowOnWo: true, delegateVC: self)
            }
        }else{
            DispatchQueue.main.async {
    ...
    @IBAction func transferBtnAction(_ sender: Any) {
        menuDataModel.uniqueInstance.presentWorkOrderTransferScreen(vc: self, rejectStr: "Transfer", presentPrsn: self.responsiblePerson, priorityVal: singleWorkOrder.Priority)
    }
        ...
        if isActiveWorkOrder == true || selectedOperationNumber.contains(find: "L"){
            ...
            if let workFlowObj = workFlowResp as? LtWorkFlowModel {
                ...
                if workFlowObj.ActionType == "Screen" {
                    ...
                    menuDataModel.presentCreateOperationScreen(vc: self, delegate: true, operationCls: self.operationVCModel.singleOperationArray[0], isFromEdit: true, WOrderType: singleWorkOrder.OrderType as String)
                }
            }
        }else {
            if WORKORDER_ASSIGNMENT_TYPE == "2" || WORKORDER_ASSIGNMENT_TYPE == "4" || WORKORDER_ASSIGNMENT_TYPE == "5"{
    ...
    @IBAction func addOperationButtonAction(sender: AnyObject) {
        ...
        if isActiveWorkOrder == true || singleWorkOrder.WorkOrderNum.contains(find: "L") {
            ...
            if let workFlowObj = workFlowResp as? LtWorkFlowModel {
                mJCLogger.log("Response :\(workFlowObj)", Type: "Debug")
                if workFlowObj.ActionType == "Screen" {
                    menuDataModel.presentCreateOperationScreen(vc: self, delegate: true, operationNmbr: "L\(String.random(length: 3, type: "Number"))")
                }
            }
        }else {
            if WORKORDER_ASSIGNMENT_TYPE == "2" || WORKORDER_ASSIGNMENT_TYPE == "4" || WORKORDER_ASSIGNMENT_TYPE == "5"{
    ...
    func customerInfoNotificationAction() {
        ...
        if DeviceType == iPad{
            if self.oprWorkOrderDetails.NotificationNum != ""{
                menuDataModel.uniqueInstance.presentSingleNotificationScreen(vc: self, selectedNONumber:  self.oprWorkOrderDetails.NotificationNum)
            }else{
    ...
    @objc func assetEquipmentButtonAction(sender : UIButton) {
        ...
        if sender.titleLabel?.text != "" && sender.titleLabel?.text != nil {
            if DeviceType == iPad{
                menuDataModel.uniqueInstance.presentFlocEquipDetialsScreen(vc: self, flocOrEquipObjType: "equip", flocOrEquipObjText: sender.titleLabel?.text, classificationType: "Workorder")
            }
        }else{
    ...
    @objc func assetFunctionLocationButtonAction(sender : UIButton) {
        ...
        if sender.titleLabel?.text != "" && sender.titleLabel?.text != nil {
            if DeviceType == iPad{
                menuDataModel.uniqueInstance.presentFlocEquipDetialsScreen(vc: self, flocOrEquipObjType: "floc", flocOrEquipObjText: sender.titleLabel?.text, classificationType: "Workorder")
            }
        }else{
            mJCLogger.log("Functional_Location_Not_Found".localized(), Type: "Debug")
            mJCAlertHelper.showAlert(self, title: MessageTitle, message: "Functional_Location_Not_Found".localized(), button: okay)
