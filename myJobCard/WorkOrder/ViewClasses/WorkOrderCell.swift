...
class WorkOrderCell: UITableViewCell {
    ...
    var noModelClass: NotificationModel?{
        didSet{
            notificationCellConfiguration()
        }
    }



    var mapWoModelClass: WoHeaderModel?{
        didSet{
            mapWorkorderCellWoConfiguration()
        }
    }
    var superWoModelClass: WoHeaderModel?{
        didSet{
            superWorkorderCellConfiguration()
        }
    }
    var superMapModelClass: WoHeaderModel?{
        didSet{
            superMapCellConfiguration()
        }
    }
    var teamMastModelClass: SupervisorTechnicianModel?{
        didSet{
            teamCellConfiguration()
        }
    }
    var assetMapMastModelClass: WoHeaderModel?{
        didSet{
            assetMapWorkorderCellConfiguration()
        }
    }
    var assetMapNoMastModelClass: NotificationModel?{
        didSet{
            assetMapNotificationCellConfiguration()
        }
    }
    var onlineWorkOrderClass: WoHeaderModel?{
        didSet{
            onlineWorkorderCellConfiguration()
        }
    }
    var onlineNotificationClass: NotificationModel?{
        didSet{
            onlineNotificationCellConfiguration()
        }
    }
    ...
    func notificationCellConfiguration() {
        
        self.backGroundView.layer.cornerRadius = 3.0
        self.backGroundView.layer.shadowOffset = CGSize(width: 3, height: 3)
        self.backGroundView.layer.shadowOpacity = 0.2
        self.backGroundView.layer.shadowRadius = 2
        if let notificationCls = noModelClass{
            if masterViewModel.woNoArray.count == masterViewModel.woNoListArray.count{
                masterViewModel.skipvalue = masterViewModel.woNoListArray.count
            }
            let arr = statusCategoryArr.filter{$0.StatusCode == notificationCls.MobileStatus}
            if arr.count > 0{
                if let statusImage = UIImage(named: arr[0].ImageResKey) {
                    self.statusImageView.image = statusImage
                }else{
            ...
            self.priorityImageView.image = myAssetDataManager.getPriorityImage(priority: notificationCls.Priority)
            self.shortTextLabel.text = "\(notificationCls.ShortText)"
            self.functionalLocLabel.text = "Floc: \(notificationCls.FunctionalLoc)"
            self.equipmentLabel.text = "Eqp: \(notificationCls.Equipment)"
            self.numberLabel.text = "\(notificationCls.Notification)"
            if globalNoAttachmentArr.contains(notificationCls.Notification){
                self.attachmentImageView.isHidden = false
            }else{
                self.attachmentImageView.isHidden = true
            }
            if notificationCls.EnteredBy == userDisplayName && DOWNLOAD_CREATEDBY_NOTIF == "X"{
                self.createdByMeImageView.isHidden = false
            }else {
                self.createdByMeImageView.isHidden = true
            }
            if Show_Postal_Code == true{
                if notificationCls.PostalCode != ""{
                    self.postalView.isHidden = false
                    self.postalCodeLabel.text = notificationCls.PostalCode
                }else{
                    self.postalView.isHidden = true
                    self.postalCodeLabel.text = ""
                }
            ...
            if notificationCls.Breakdown == "X" || notificationCls.Breakdown == "x"{
                self.breakDownImageView.isHidden  = false
            }else {
                self.breakDownImageView.isHidden  = true
            }
            if DeviceType == iPhone{
                self.orderTypeLabel.isHidden = false
                self.orderTypeLabel.text = notificationCls.NotificationType
                if((UserDefaults.standard.value(forKey:"active_details")) != nil){
                    let activedetails = UserDefaults.standard.value(forKey: "active_details") as! Dictionary<String, Any>
                    if let notificationNum = activedetails["Notification"]  as? String{
                        if notificationNum == notificationCls.Notification {
                            self.activeInfoButton.isHidden = false
                            self.transperentView.isHidden = false
                            self.isSelectedCellView.isHidden = false
                        }else{
                ...
                if(notificationCls.isSelectedCell) {
                    self.isSelectedCellView.isHidden = false
                    self.transperentView.isHidden = false
                }else{
                    self.transperentView.isHidden = true
            ...
            if notificationCls.RequiredStartDate != nil{
                RequiredStartDate = notificationCls.RequiredStartDate!.toString(format: .custom(localDateFormate), timeZone: .utc, locale: .current)
                self.dateLabel.text = "Start_Date".localized() + " : ".localized() + RequiredStartDate
            }
        }
    }
    
    func mapWorkorderCellWoConfiguration() {

        self.backGroundView.layer.cornerRadius = 3.0
        self.backGroundView.layer.shadowOffset = CGSize(width: 3, height: 3)
        self.backGroundView.layer.shadowOpacity = 0.2
        self.backGroundView.layer.shadowRadius = 2
        if let workorderCls = mapWoModelClass{
    ...
    func superWorkorderCellConfiguration() {
        ...
        if let workorderCls = superWoModelClass {
            let status = WorkOrderDataManegeClass.uniqueInstance.setWorkOrderStatus(userStatus: workorderCls.UserStatus, mobileStatus: workorderCls.MobileObjStatus, woClass: workorderCls)
            let arr = statusCategoryArr.filter{$0.StatusCode == status}
            if arr.count > 0{
                if let statusImage = UIImage(named: arr[0].ImageResKey) {
            ...
            self.numberLabel.text = workorderCls.WorkOrderNum
            self.shortTextLabel.text = workorderCls.ShortText
            self.functionalLocLabel.text = "Floc / Eqp: \(workorderCls.FuncLocation) / \(workorderCls.EquipNum)"
            self.dateLabel.text = "Techinician".localized() + ":" + " \(workorderCls.TechnicianName)"

            if workorderCls.BasicStrtDate != nil{
                let startDate = workorderCls.BasicStrtDate!.toString(format: .custom(localDateFormate), timeZone: .utc, locale: .current)
                self.equipmentLabel.text = "Start_Date".localized() + " : " + startDate
            }else{
                self.equipmentLabel.text = "Start_Date".localized() + " : "
            }
            if DeviceType == iPhone{
                self.orderTypeLabel.isHidden = false
                self.orderTypeLabel.text = workorderCls.OrderType
            ...
            if(workorderCls.isSelectedCell) {
                TechnicianName = self.dateLabel.text!
                self.superViewModel.getRecordpointdata(workorder: workorderCls)
                if DeviceType == iPad{
                    self.superViewModel.vc.performSegue(withIdentifier: "showDetail", sender: self)
                }
                self.isSelectedCellView.isHidden = false
                self.transperentView?.isHidden = false
            }else {
            ...
            if workorderCls.hasAttachment == true {
                self.attachmentImageView.isHidden = false
            }else {
                self.attachmentImageView.isHidden = true
            }
    ...
    func superMapCellConfiguration() {
        ...
        if let workorderCls = superMapModelClass {
            let status = WorkOrderDataManegeClass.uniqueInstance.setWorkOrderStatus(userStatus: workorderCls.UserStatus, mobileStatus: workorderCls.MobileObjStatus, woClass: workorderCls)
            let arr = statusCategoryArr.filter{$0.StatusCode == status}
            if arr.count > 0{
                if let statusImage = UIImage(named: arr[0].ImageResKey) {
...
//            if selectedWOIndex == indexpath.row {
//                self.isSelectedCellView.isHidden = false
//                self.transperentView?.isHidden = false
//            }else {
//                self.transperentView?.isHidden = true
...
//            }
    ...
    func teamCellConfiguration() {
        if let alltechclass = teamMastModelClass {

            self.backGroundView.layer.cornerRadius = 3.0
            self.backGroundView.layer.shadowOffset = CGSize(width: 3, height: 3)
            self.backGroundView.layer.shadowOpacity = 0.2
            self.backGroundView.layer.shadowRadius = 2
            self.numberLabel.text = alltechclass.Name
            ...
            self.shortTextLabel.text = "Techcinian_Id".localized() + ": \(alltechclass.Technician)"
            self.functionalLocLabel.text = "Total_Workorders".localized() + ": \(alltechclass.TechnicianWorkordercount)"
        }
    }
    
    func assetMapWorkorderCellConfiguration() {

        if let workorderCls = assetMapMastModelClass {
...
//                if selectedNOIndex == indexpath.row {
//                    self.isSelectedCellView.isHidden = false
//                    self.transperentView.isHidden = false
//                }else {
//                    self.transperentView.isHidden = true
...
//                }
    ...
    func assetMapNotificationCellConfiguration() {

        self.backGroundView.layer.cornerRadius = 3.0
        self.backGroundView.layer.shadowOffset = CGSize(width: 3, height: 3)
        self.backGroundView.layer.shadowOpacity = 0.2
        self.backGroundView.layer.shadowRadius = 2
        if let notificationCls = assetMapNoMastModelClass{
...
//            }
    ...
    func onlineWorkorderCellConfiguration(){

        self.backGroundView.layer.cornerRadius = 3.0
        self.backGroundView.layer.shadowOffset = CGSize(width: 3, height: 3)
        self.backGroundView.layer.shadowOpacity = 0.2
        self.backGroundView.layer.shadowRadius = 2
        self.detailArrowImageView.isHidden = true
        if let workorderCls = onlineWorkOrderClass{
            let status = WorkOrderDataManegeClass.uniqueInstance.setWorkOrderStatus(userStatus: workorderCls.UserStatus, mobileStatus: workorderCls.MobileObjStatus, woClass: workorderCls)
            let arr = statusCategoryArr.filter{$0.StatusCode == status}
            if arr.count > 0{
                if let statusImage = UIImage(named: arr[0].ImageResKey) {
    ...
    func onlineNotificationCellConfiguration(){
        self.detailArrowImageView.isHidden = true
        self.backGroundView.layer.cornerRadius = 3.0
        self.backGroundView.layer.shadowOffset = CGSize(width: 3, height: 3)
        self.backGroundView.layer.shadowOpacity = 0.2
        self.backGroundView.layer.shadowRadius = 2
        if let notificationCls = onlineNotificationClass{
            let arr = statusCategoryArr.filter{$0.StatusCode == notificationCls.MobileStatus}
            if arr.count > 0{
                if let statusImage = UIImage(named: arr[0].ImageResKey) {
                    self.statusImageView.image = statusImage
