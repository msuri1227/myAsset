...
class TotalOperationCountCell: UITableViewCell {
    ...
    var componentViewModel = ComponentsViewModel()
    ...
    var totalCompListIphoneClass: WoComponentModel?{
        didSet{
            compListDataConfiguration()
        }
    }
    var woInspOperationModelClass: InspectionOperationModel? {
        didSet{
            woInspectionOperationConfiguration()
        }
    }
    ...
    func woInspectionOperationConfiguration() {
        mJCLogger.log("Starting", Type: "info")
        if inspViewModel.selectedOprCellIndex == indexPath.row{
            transPerantView.isHidden = false
            sideIndicatorView.backgroundColor = appColor
        }else{
            transPerantView.isHidden = true
            sideIndicatorView.backgroundColor = UIColor.clear
        }
        if DeviceType == iPad{
            clockImgWidthConstraint.constant = 0.0
        }
        selectOperationButtonLeadingConstant.constant = 0.0
        operationCompleteImageWidthConstraint.constant = 0.0
        selectCheckBoxWidthConst.constant = 0.0
        operationNameLabel.text = woInspOperationModelClass?.Operation
        DescriptionLabel.text = woInspOperationModelClass?.ShortText
        self.selectOpearettionButton.isHidden = false
        selectOpearettionButton.tag = indexPath.row
        selectOpearettionButton.addTarget(self, action: #selector(inspOperationButtonClicked), for: .touchUpInside)
        mJCLogger.log("Ended", Type: "info")
    }
    //MARK:- Inspection Operation select Button Action..
    @objc func inspOperationButtonClicked(btn:UIButton)  {
        mJCLogger.log("Starting", Type: "info")
        if DeviceType == iPad{
            let operation =  inspViewModel.inspOprArray[btn.tag]
            inspViewModel.fromResultsScreen = false
            inspViewModel.selectedOprCellIndex = btn.tag
            inspViewModel.getInspectionPoints(lot: operation.InspectionLot, Opr: operation.Operation)
            inspViewModel.inspVc?.operationListTableView.reloadData()
        }else{
            inspViewModel.inspOprVc?.updateInspOperationDetails(tagValue: btn.tag)
        }
        mJCLogger.log("Ended", Type: "info")
    }
    
    func compListDataConfiguration() {
        mJCLogger.log("Starting", Type: "info")
        if let componentModeClass = totalCompListIphoneClass {
            ...
            self.backGroundView.layer.shadowRadius = 2
            self.operationCompleteImageWidthConstraint.constant = 0.0
            self.operationNameLabel.text = "Component".localized() + ":  \(componentModeClass.Item)"
            self.DescriptionLabel.text = "Description".localized() + ": \(componentModeClass.MaterialDescription)"
            self.dueDateLabel.text = "Required_Qty_(PC)".localized() + ":  \(componentModeClass.ReqmtQty)"
            self.WithdranQtyLabel.isHidden = false
            self.WithdranQtyLabel.text = "Withdrawn_Qty_(PC)".localized() + " : \(componentModeClass.WithdrawalQty)"
            self.selectCheckBoxButton.tag = indexPath.row
            self.selectCheckBoxButton.addTarget(self, action: #selector(self.selectComponentCheckBox(btn:)), for: .touchUpInside)
            self.selectCheckBoxWidthConst.constant = 20.0
            if self.componentViewModel.selectedComponentArray.contains(componentModeClass){
                self.selectCheckBoxButton.isSelected = true
            }else{
                self.selectCheckBoxButton.isSelected = false
            }
            if selectedComponentNumber == componentModeClass.Item {
                if componentModeClass.isSelected == true {
                    transPerantView.isHidden = false
                    self.componentViewModel.componentsVC?.selectedComponent = componentModeClass.Item
                    self.componentViewModel.did_DeSelectedCell = indexPath.row
                    self.componentViewModel.selectedComponentClass = componentModeClass
                }
                else {
                    transPerantView.isHidden = true
                }
            ...
            let  withdrawalQty = Int(truncating: componentModeClass.WithdrawalQty)
            let  reqmtQty = Int(truncating: componentModeClass.ReqmtQty)
            if Int(truncating: componentModeClass.WithdrawalQty ) > 0 {
                if componentModeClass.WithdrawalQty == componentModeClass.ReqmtQty || withdrawalQty > reqmtQty{
                    sideIndicatorView.isHidden = true
                    operationCompleteImageWidthConstraint.constant = 20.0
                        ...
                        if let featureDict = (featureListArr as! NSArray)[0] as? NSMutableDictionary{
                            if let featurelist = featureDict.allKeys as? [String]{
                                if featurelist.contains("COMPONENT"){
                                    ...
                                    if mandLevel == "1"{
                                        sideIndicatorView.isHidden = false
                                        sideIndicatorView.backgroundColor = filledCountColor
                                    }else{
                                        sideIndicatorView.isHidden = false
                                        sideIndicatorView.backgroundColor = UIColor.red
                                    }
                                }else{
                                    sideIndicatorView.isHidden = false
                                    sideIndicatorView.backgroundColor = filledCountColor
    ...
    @objc func selectComponentCheckBox(btn:UIButton)  {
        mJCLogger.log("Starting", Type: "info")
        self.componentViewModel.componentsIPhoneVC?.selectComponentCheckBox(btn: btn)
        mJCLogger.log("Ended", Type: "info")
    }
