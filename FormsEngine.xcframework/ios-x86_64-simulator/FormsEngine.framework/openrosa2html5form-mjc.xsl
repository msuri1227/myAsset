...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:func="http://www.**.com"
  ...
  <xsl:variable name="translated">
    <!-- assumes that either a whole form is translated or nothing (= real life) -->
    <xsl:if test="count(/h:html/h:head/xf:model/xf:itext/xf:translation) &gt; 1" >
      <xsl:value-of select="string('true')" />
      <!-- no time to figure out how to use real boolean values -->
    </xsl:if>
  </xsl:variable>
  ...
  <xsl:template match="/">
    <xsl:if test="not(function-available('exsl:node-set'))">
      <xsl:message terminate="no">FATAL ERROR: exsl:node-set function is not available in this XSLT processor</xsl:message>
    </xsl:if>
    <xsl:if test="not(function-available('str:replace'))">
      <xsl:message terminate="no">FATAL ERROR: str:replace function is not available in this XSLT processor</xsl:message>
    </xsl:if>
    <xsl:if test="not(function-available('dyn:evaluate'))">
      <xsl:message terminate="no">FATAL ERROR: dyn:evaluate function is not available in this XSLT processor</xsl:message>
    </xsl:if>
    <xsl:if test="not(function-available('str:tokenize'))">
      <xsl:message terminate="no">FATAL ERROR: str:tokenize function is not available in this XSLT processor</xsl:message>
    </xsl:if>
    <xsl:for-each select="/h:html/h:head/xf:model/xf:bind">
      <xsl:if test="not(substring(./@nodeset, 1, 1) = '/')">
        <xsl:message terminate="no">
          WARNING: Found binding(s) with relative nodeset attribute <!--on element: <xsl:value-of select="./@nodeset" />--> (form may work correctly if relative nodesets were used consistently throughout xml form in bindings as well as body, otherwise it will certainly be messed up).
        </xsl:message>
      </xsl:if>
    </xsl:for-each>
        ...
        <xsl:attribute name="id">
          <xsl:choose>
            <xsl:when test="/h:html/h:head/xf:model/xf:instance[1]/child::node()/@id">
              <xsl:value-of select="translate(/h:html/h:head/xf:model/xf:instance/child::node()/@id, ' ', '_' )" />
              <!-- not smart! -->
            </xsl:when>
  ...
  <!--[not(self::xf:model/xf:bind[@jr:preload])]" />-->
  ...
  <xsl:template match="xf:group">
    ...
    <xsl:comment>
      end of group <xsl:value-of select="@nodeset" />
    </xsl:comment>
  </xsl:template>
  ...
  <xsl:template name="appearance">
    <xsl:if test="@appearance">
      ...
      <xsl:value-of select="concat('or-appearance-', normalize-space(translate(@appearance, $upper-case, $lower-case)), ' ')"/>
      <!--</xsl:for-each>-->
    </xsl:if>
  ...
  <xsl:template match="xf:itemset" mode="labels">
    ...
    <xsl:variable name="instance-path" select="substring-after(@nodeset, ')')" />
    ...
    <span class="itemset-labels">
      ...
      <xsl:choose>
        <xsl:when test="contains($label-ref, 'jr:itext(')">
          ...
          <xsl:call-template name="mjc_casc">
            <xsl:with-param name="instance_id" select="$instance-id"/>
            <xsl:with-param name="node-name-label" select="$label-node-name"/>
  ...
  <xsl:template name="mjc_casc" match="/h:html/h:head/xf:model/xf:instance">
    <xsl:param name="instance_id"/>
    <xsl:param name="node-name-label" />
    <xsl:for-each select="/h:html/h:head/xf:model/xf:instance">
      <xsl:if test="@id=$instance_id">
        <xsl:for-each select="xf:root/xf:item">
                     
            <span lang="default" class="option-label active" data-itext-id="{xf:itextId}">
  ...
  <xsl:template name="text-content">
    ...
    <xsl:apply-templates />
    <!-- call xf:output template if output is present -->
  </xsl:template>
        ...
        <xsl:choose>
          <xsl:when test="@form = 'image'" >
            ...
            <xsl:choose>
              ...
              <xsl:when test="not(../xf:value[@form = 'big-image'])" >
                <xsl:call-template name="image">
                  <xsl:with-param name="active" select="$active"/>
                  <xsl:with-param name="notext" select="$notext"/>
                  <xsl:with-param name="lang" select="$lang"/>
  ...
  <xsl:template name="html_type">
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
        ...
        <xsl:message terminate="no">
          ERROR: Unsupported data type '<xsl:value-of select="$xml_type"/>' found.
        </xsl:message>
      </xsl:otherwise>
    </xsl:choose>
  </xsl:template>
