    ...
    func timeSheetEditClick(sender : UIButton) {
        ...
        if let workFlowObj = workFlowResp as? LtWorkFlowModel {
            if workFlowObj.ActionType == "Screen" {
                ...
                addTimeEntryVC.timeSheetDelegate = self.vc
                addTimeEntryVC.timeSheetClass = self.timeSheetArray[sender.tag]
                addTimeEntryVC.cellIndex = sender.tag
                addTimeEntryVC.modalPresentationStyle = .fullScreen
                self.vc.present(addTimeEntryVC, animated: false) {}
            }
        }
        mJCLogger.log("Ended", Type: "info")
    }
