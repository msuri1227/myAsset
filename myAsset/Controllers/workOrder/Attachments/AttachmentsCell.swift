                    ...
                    if myAsset.fileManager.fileExists(atPath: newfilePath) {
                        if imgExtensions.contains(where: {$0.caseInsensitiveCompare(attachmentClass.Extension) == .orderedSame}) || imgExtensions.contains(where: {$0.caseInsensitiveCompare(attachmentClass.MimeType) == .orderedSame}){
                            ...
                            if getimage != nil {
                                ...
                                attachmentViewer.attachmentImage = getimage!
                                attachmentViewer.modalPresentationStyle = .fullScreen
                                attachmentsViewModel.vc!.present(attachmentViewer, animated: false)
                            }else{
                            ...
                            let attachmentViewer = ScreenManager.getAttachmentViewerScreen()
                            attachmentViewer.attachmenttype = "pdf"
                            let url2 = NSURL(fileURLWithPath: newfilePath)
                            attachmentViewer.pdfURL = url2
                            attachmentViewer.modalPresentationStyle = .fullScreen
                            attachmentsViewModel.vc!.present(attachmentViewer, animated: false)
                        }else{
                            let url : NSURL = NSURL(fileURLWithPath: newfilePath)
                                ...
                                if let status = workorderDict["Status"] as? String{
                                    if status == "Success"{
                                        if let dict = workorderDict["Response"] as? NSMutableDictionary{
                                            let responseDict = formateHelperClass.getListInFormte(dictionary: dict, entityModelClassType: attachmentContentModel.self)
